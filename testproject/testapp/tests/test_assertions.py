...
class StatusCodeAssertionsMixinTest(
    assertions.StatusCodeAssertionsMixin, testcases.ViewTestCase
):
    view_class = MockView
...
class EmailMockView(generic.View):
    def get(self, *args, **kwargs):
        for _ in range(int(self.request.GET.get("send"))):
            mail.send_mail(
                "test", "tasty test", "glados@aperture.edu", ["you@testingchamber.com"]
            )
        return HttpResponse()
...
class MessagesAssertionsMixin(
    assertions.MessagesAssertionsMixin, testcases.ViewTestCase
):
...
class InstanceAssertionsMixinTest(assertions.InstanceAssertionsMixin, TestCase):
    ...
    def test_assert_instance_does_not_exist_raises_assertion_error_when_instance_found(
        self,
    ):
        models.MockModel.objects.create(field="value")
